			...
			<div>
				...
				<Segment compact inverted>
					{this.state.edit
					 ?
					 <>
						 {this.props.entries.map(b => <Button basic color='blue' key={b.id} onClick={this.editFalse}>Edit
							 → {b.title}</Button>)}
					 </>
					 :
					 <>
						 {this.props.entries.map(b => <Button basic color='blue' key={b.id} onClick={() => this.editTrue(b)}>Edit
							 → {b.title}</Button>)}
					 </>
					}
					<Container>
						{this.props.entries.map(b =>
							<Button basic color='red' key={b.id} onClick={() => this.props.removeEntry(b.id)}>
								Delete → {b.title}
							</Button>
						)}
					</Container>
